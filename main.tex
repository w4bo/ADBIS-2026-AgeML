...
%
\institute{Department of Computer Science and Engineering, Alma Mater Studiorum\textemdash{}Universit\`a di Bologna, Cesena, Italy\\
...
%
Although \ac{AutoML} has reduced the cost of model selection and hyper-parameter tuning,
...
%
From the specification, a search space for pipeline generation is algorithmically constructed and explored to ensure reproducibility and controlled automation.
%
Then, \acp{LLM} operate within a controlled agent loop to interpret specifications, generate and validate pipelines, and produce structured explanations for them.
%
...
\begin{figure*}%[t]
  ...
  \caption{
    ...
    %
    It lets agents (green) generate end-to-end \ac{ML} code leveraging well-known \ac{ML} libraries as tools.
    %
    \Acp{LLM} are used by agents as a service in order to generate code and reason about the data.
    %
...
%
In this paradigm,
the role of the data scientist extends beyond model selection and hyper-parameter tuning to the design of effective data preprocessing, feature engineering, and validation.
%
Injecting domain knowledge and a-priori experience into \ac{ML} pipelines is therefore essential.
%
However,
...
and manual orchestration of workflow components\textemdash{}all tasks that could be automated by coding frameworks
...
%
Traditional \ac{AutoML} approaches primarily concentrate on model selection and hyper-parameter optimisation,
...
\begin{inlinelist}
    ...
    %
    \item reduces operational load for data scientists
    (by automating generation and execution of low-level coding and tuning tasks),
    %
    and
    %
    \item supports modularity and reusability of knowledge
    (data scientists' knowledge can be encoded, extended, and shared over time).
\end{inlinelist}
\ourapproach{}
(\cref{fig:research-line})
delegates the design and implementation of \ac{ML} pipelines to a \ac{MAS}
where agents interleave symbolic reasoning, intuitive reasoning, text generation
...
\Cref{sec:agentic-architecture-for-specification-driven-automl} introduces the architecture of \ourapproach{}, whereas \Cref{sec:experimental-evaluation} evaluates it also against state-of-the-art approaches.
\Cref{sec:conclusion} concludes the paper and discusses future work.
...
improving predictive performance through the exploration of model families, preprocessing strategies, and hyper-parameters.
%
...
and
the \ac{HPO},
by searching for the best algorithm and also its hyper-parameters for a given dataset.
%
Other frameworks
(e.g., Auto-Keras~\cite{DBLP:conf/kdd/JinSH19})
focus more on searching for deep learning models and support multimodal and multitask learning.
%
More recently,
...
in particular a lack of transparent optimisation processes and insufficient integration of constraints and domain semantics from data scientists,
...
while preserving full end-to-end automation and integrating \emph{human-centered design}.
%
...
% \end{inlinelist}
% %
% Although \ac{AutoML} has substantially advanced algorithm selection and hyper-parameter tuning,
...
%     %
%     \item the corresponding hyper-parameters,
...
% and which hyper-parameters to select.
% %
% Operationally,
...
% uses symbolic reasoning to select admissible steps, algorithms, and hyper-parameters,
...
% %
% This allows the inspection of not just the final output \textendash{} hence, the best model \textendash{} but also of the intermediate results consisting of different pipelines, along with their performance and artefacts.
...
\begin{figure*}[t]
  ...
  \caption{
    ...
    Androids represent agents and open-headed dashed arrows denote interactions with \acp{LLM}.
  }
...
%
This kind of specification is inspired by SKI-Lang~\cite{skilang}
where a YAML-like language is used to encode knowledge to be injected into a neural predictor.
%
The file comprises four main sections:
\texttt{pipeline},
\texttt{ordering},
...
\Cref{lst:spec-ordering} shows an excerpt from the specification.
\texttt{pipeline} specifies the admissible steps, their candidate implementations, and their hyper-parameter domains.
...
\texttt{budgets} defines execution budgets and resource limits for an automated search process, allowing at most 5 candidate pipelines to be evaluated, and up to 10 attempts to generate valid pipeline code.
...
so that downstream components receive only pipelines that are structurally valid with respect to the specification.

A pipeline can still be seen as a finite sequence of step\textendash{}candidate pairs:
...
tailored to the input YAML specification file.
%
...
The full \ac{CSP} model is reported in the supplementary materials,
...
% \textendash{} other than explicit and inspectable --:
...
The pipeline executor is responsible for synthesizing and executing pipelines.
...
the executor agent produces a Python script implementing exactly the candidate algorithms in the prescribed order.
%
The generated script is parametric with respect to the hyper-parameters
associated with the candidates in the target pipeline according to the user-provided specification.
%
...
\begin{figure}[t]
  ...
  State machine governing the behavior of executor agents.
    ...
    \textendash{} multiple times, exploring different hyper-parameter assignments \textendash{},
    and finally produces a textual explanation of the outcome.
    % Dashed edges denote retry paths triggered by failed validation or failed execution.
  }
  \label{fig:executor-state-machine}
\end{figure}

\Cref{fig:executor-state-machine} depicts the state machine governing the behaviour of executor agents.
%
...
with a fixed interface and well-defined responsibility.
%
...
\begin{inlinelist}
  ...
  and
  \item a matching problem in which the generated code is matched against a number of regular expressions,
...
%
In doing so,
the agent steers the \ac{LLM} to summarise the whole conversational context,
...
currently implemented using \texttt{MLflow}~\cite{zaharia2018accelerating},
in a hierarchical way\footnote{\texttt{MLflow} acts as a model/pipeline registry that treats AI models as cataloged assets with governed access, auditing, and lineage.}.
%
...
%
Second,
it provides the evaluator agent with a structured metadata organisation,
...
\textemdash{}in compliance with budget limitations.

To this end,
the evaluator traverses the aforementioned hierarchy of logged artefacts
(see \cref{fig:logging-hierarchy})
from the leaves to the root,
assessing the performance metrics of the trained models on the test set and propagating the best values up to the experiment level.
%
...
We empirically evaluate the implementation of \ourapproach{},
both in \emph{absolute} terms
-- by assessing the predictive performance achieved by \ourapproach{} on a collection of well-known benchmark datasets \textendash{},
and in \emph{relative} terms%
\textemdash{}by comparing the performance of \ourapproach{} against a state-of-the-art \ac{AutoML} system
...
and our implementations of the pipeline planner, pipeline executor, and evaluator agents,
...
along with specifications and automation scripts for reproducing our experiments,
...
%
The experimental evaluation relies on \acp{LLM} to support
\begin{inlinelist}
    \item semantic constraints' translation,
    \item code generation,
    \item validation, and
    \item explanation
\end{inlinelist}
within the executor agents.
%
In particular,
...
%   \item hyper-parameter exploration within each pipeline,
...
respectively,
across six classification and three regression datasets.
%
%For the baseline,
...
Both models generate pipelines that achieve competitive performance compared with the baselines,
...
although with slightly higher variance and a more noticeable performance degradation on \textit{Ames Housing}.
...
%
This reduced exploration may directly impact performance,
...
Future work will explore several directions to enhance the system's capabilities and applicability.
...
This would improve accessibility for non-experts and reduce the effort required from experienced practitioners.
...
\item Extending evaluation across multiple \acp{LLM}, since the architecture already supports the easy integration of new providers but the current evaluation relies on a single provider (Google) and a limited set of models.
...
who kindly provided us with access to his credits for \acp{LLM} usage on Google AI studio,
